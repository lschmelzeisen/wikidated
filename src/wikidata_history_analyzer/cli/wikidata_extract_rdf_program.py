...
from typing import AbstractSet, Counter, Tuple, cast
...
class WikidataExtractRdfProgram(WikidataRdfRevisionProgram):
    class Config(ProgramConfig):
        title = "wikidata-extract-rdf"
        version = wikidata_history_analyzer.__version__
        description = "Extract RDF revisions."
...
def main(*args: str) -> None:
    if not args:
        args = tuple(argv[1:])
    WikidataExtractRdfProgram.init(*args).run()
